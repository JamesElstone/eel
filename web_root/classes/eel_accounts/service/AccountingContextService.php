...
final class AccountingContextService implements \SiteContextProviderInterface
{
    ...
    private function resolveSelection(\RequestFramework $request, array $pageContext = []): array
    {
        ...
        $companyRows = (new \eel_accounts\Repository\CompanyRepository())->fetchCompanySelectorRows();
        $requestedCompanyId = $this->contextId($pageContext, 'company_id');
        if ($requestedCompanyId <= 0) {
            $requestedCompanyId = $this->requestContextId($request, 'company_id', 'company_id');
        }

        $companyId = $this->resolveCompanyId($companyRows, $requestedCompanyId);
        $company = $companyId > 0 ? (new \eel_accounts\Repository\CompanyRepository())->fetchCompanyDetails($companyId) : null;
        if ($company === null) {
            $companyId = 0;
        }

        $accountingPeriods = $companyId > 0 ? (new \eel_accounts\Repository\AccountingPeriodRepository())->fetchAccountingPeriods($companyId) : [];
        $requestedAccountingPeriodId = $this->contextId($pageContext, 'accounting_period_id');
        if ($requestedAccountingPeriodId <= 0) {
            $requestedAccountingPeriodId = $this->requestContextId($request, 'accounting_period_id', 'accounting_period_id');
        }
        $accountingPeriodId = $this->resolveAccountingPeriodId($accountingPeriods, $requestedAccountingPeriodId);
        $accountingPeriod = $accountingPeriodId > 0 ? (new \eel_accounts\Repository\AccountingPeriodRepository())->fetchAccountingPeriod($companyId, $accountingPeriodId) : null;
    ...
    private function accountingPeriodOptions(array $accountingPeriods): array
    {
        ...
        foreach ($accountingPeriods as $accountingPeriod) {
            ...
            if ($label === '') {
                ...
                $label = $start !== '' && $end !== ''
                    ? \eel_accounts\Service\TaxPeriodService::accountingPeriodLabel($start, $end)
    ...
    private function validAccountingPeriodId(int $companyId, int $accountingPeriodId): int
    {
        ...
        return (new \eel_accounts\Repository\AccountingPeriodRepository())->fetchAccountingPeriod($companyId, $accountingPeriodId) !== null ? $accountingPeriodId : 0;
    }
