...
final class CompanyAccountNominalService
{
    ...
    public function resolveNominalForAccountInput(array $input): array
    {
        ...
        return $this->createNominalForAccount([
            ...
            'account_type' => (string)($input['account_type'] ?? \eel_accounts\Service\CompanyAccountService::TYPE_BANK),
        ]);
    }

    private function createNominalForAccount(array $account): array
    {
        $accountType = (string)($account['account_type'] ?? \eel_accounts\Service\CompanyAccountService::TYPE_BANK);
    ...
    private function definitionForAccountType(string $accountType, array $settings = []): ?array
    {
        ...
        return match ($accountType) {
            \eel_accounts\Service\CompanyAccountService::TYPE_BANK => [
                'start' => $bankStart,
                'end' => $bankStart + 98,
                'account_type' => 'asset',
                'subtype_code' => 'bank',
            ...
            \eel_accounts\Service\CompanyAccountService::TYPE_TRADE => [
                'start' => $tradeStart,
                'end' => $tradeStart + 98,
    ...
    private function settingsForAccount(array $account): array
    {
        ...
        return $companyId > 0 ? (new \eel_accounts\Store\CompanySettingsStore($companyId))->all() : [];
    }
