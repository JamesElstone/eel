...
final class ReceiptDownloadService
{
    ...
    public function downloadReceiptForTransaction(int $transactionId): array {
        ...
        $this->fileCheckService->ensureTransactionReceiptDirectory((int)$transaction['company_id']);
    ...
    private function downloadUrlToTemporaryFile(string $url): array {
        ...
        try {
            $response = \eel_accounts\Outbound\ReceiptOutbound::downloadToFile($url, $handle, $this->timeoutSeconds, $this->maxBytes);
        } catch (\Throwable $exception) {
            fclose($handle);
            @unlink($tempPath);
    ...
    private function receiptDirectoryForCompany(int $companyId): string {
        return $this->fileCheckService->getTransactionReceiptDirectory($companyId);
    }
    ...
    private function uploadsConfig(): array
    {
        ...
        $uploads['upload_base_dir'] = $this->baseDirectory;
