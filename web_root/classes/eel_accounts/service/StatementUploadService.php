...
final class StatementUploadService
{
    ...
    public function __construct(
        ...
        ?\eel_accounts\Service\ReceiptDownloadService $receiptDownloadService = null,
    ...
    public function createUploadFromHttpRequest(array $post, array $files): array {
        ...
        if ($accountingPeriodId > 0 && (new \eel_accounts\Repository\AccountingPeriodRepository())->fetchAccountingPeriod($companyId, $accountingPeriodId) === null) {
    ...
    private function applyOfflineRowUpdate(array $target, array $row, array $sourceHeaders, array $nominalIndex): array {
        ...
        if ($category !== '') {
            ...
            if ($nominalId === null) {
                ...
                if ($transactionId > 0) {
                    $categorisationService = $this->categorisationService ?? new \eel_accounts\Service\TransactionCategorisationService();
    ...
    private function buildOfflineRowKeyLookup(int $companyId): array {
        ...
        foreach ($rows as $row) {
            ...
            $key = \eel_accounts\Service\StatementCsvExportService::exportRowKey($companyId, (int)($row['upload_id'] ?? 0), (int)($row['row_number'] ?? 0), $exportRow);
