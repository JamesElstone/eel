...
function outboundHelperKeysPath(?string $overridePath = null): string {
...
function outboundHelperCredentialCatalog(?string $keysPath = null): array {
    static $catalogByPath = [];

    $path = outboundHelperKeysPath($keysPath);
    ...
    try {
        while (($row = fgetcsv($handle, 0, ',', '"', '')) !== false) {
            ...
            if (count($row) >= 6) {
                $environment = ctrl_normalise_environment_mode((string)$row[2]);
                $credential = [
                    'provider' => $provider,
                    'tag' => $tag,
...
function outboundHelperLoadCredential(string $provider, string $tag, ?string $environment = null, ?string $keysPath = null): array {
    $provider = strtoupper(trim($provider));
    $tag = strtoupper(trim($tag));
    $environment = ctrl_normalise_environment_mode($environment);
    $catalog = outboundHelperCredentialCatalog($keysPath);
...
function outboundHelperRequest(array $request): array {
    ...
    if (
        ...
        $credential = outboundHelperLoadCredential(
    ...
    if ($transport === 'soap') {
        return outboundHelperSoapRequest($request, $credential);
    }

    return outboundHelperHttpRequest($request, $credential);
}

function outboundHelperHttpRequest(array $request, ?array $credential = null): array {
    ...
    if ($authMode === 'basic_api_key') {
        ...
        [$clientId, $clientSecret] = outboundHelperResolveClientCredentials($request, $credential);
...
function outboundHelperSoapRequest(array $request, ?array $credential = null): array {
...
function outboundHelperResolveClientCredentials(array $request, ?array $credential = null): array {
