...
final class ExpenseReceiptStorageService
{
    ...
    public function __construct(?string $uploadsRoot = null, int $maxBytes = 10485760, ?FileCheckService $fileCheckService = null) {
    ...
    public function uploadReceipt(int $companyId, int $claimId, int $lineId, array $file): array {
        ...
        $this->fileCheckService->ensureExpenseReceiptDirectory((int)($line['company_id'] ?? 0));
    ...
    private function receiptDirectoryForCompany(int $companyId): string {
        return $this->fileCheckService->getExpenseReceiptDirectory($companyId);
    }

    private function relativePathForCompany(int $companyId, string $filename): string {
        return $this->fileCheckService->getExpenseReceiptRelativePath($companyId, $filename);
    }
