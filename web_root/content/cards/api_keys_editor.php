...
final class _api_keys_editorCard extends CardBaseFramework
{
    ...
    private function rows(array $rows): string
    {
        ...
        foreach ($rows as $row) {
            ...
            if ($id === '') { continue; }
            $html .= '<tr><td>' . HelperFramework::escape((string)($row['provider'] ?? '')) . '</td><td>' . HelperFramework::escape((string)($row['gateway'] ?? '')) . '</td><td>' . HelperFramework::escape((string)($row['tag'] ?? '')) . '</td><td>' . HelperFramework::escape((string)($row['environment'] ?? '')) . '</td><td>' . HelperFramework::escape((string)($row['schema'] ?? '')) . '</td><td>' . HelperFramework::escape((string)($row['url'] ?? '')) . '</td><td>' . HelperFramework::escape((string)($row['software_reference'] ?? '')) . '</td><td><button class="button button-inline" type="button" data-api-credential-edit="true" data-credential-id="' . HelperFramework::escape($id) . '" data-credential-provider="' . HelperFramework::escape((string)($row['provider'] ?? '')) . '" data-credential-gateway="' . HelperFramework::escape((string)($row['gateway'] ?? '')) . '" data-credential-tag="' . HelperFramework::escape((string)($row['tag'] ?? '')) . '" data-credential-environment="' . HelperFramework::escape((string)($row['environment'] ?? '')) . '" data-credential-schema="' . HelperFramework::escape((string)($row['schema'] ?? '')) . '" data-credential-url="' . HelperFramework::escape((string)($row['url'] ?? '')) . '" data-credential-software-reference="' . HelperFramework::escape((string)($row['software_reference'] ?? '')) . '">Edit</button></td></tr>';
        }
        return $html;
    }
