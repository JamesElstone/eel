...
final class _filing_evidence_section_detailCard extends CardBaseFramework
{
    ...
    public function render(array $context): string
    {
        ...
        if (empty($model['available'])) { return '<div class="helper">' . \eel_accounts\Support\Utf8::html((string)(($model['errors'] ?? [])[0] ?? 'Frozen section evidence is unavailable.')) . '</div>'; }
        $section = (array)($model['section'] ?? []); $rows = '';
        foreach ((array)($model['rows'] ?? []) as $row) {
            $rows .= '<tr><td><pre class="helper">' . \eel_accounts\Support\Utf8::html(\eel_accounts\Support\Utf8::json($row, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) ?: '{}') . '</pre></td></tr>';
        }
        ...
        if ((int)($pagination['page_count'] ?? 1) > 1) {
            ...
            foreach ([max(1, (int)$pagination['page'] - 1) => 'Previous', min((int)$pagination['page_count'], (int)$pagination['page'] + 1) => 'Next'] as $targetPage => $label) {
                ...
                $navigation .= '<form method="post" action="?page=filing_evidence" data-ajax="true">'
                    ...
                    . '<input type="hidden" name="evidence_bundle_id" value="' . (int)($state['bundle_id'] ?? 0) . '"><input type="hidden" name="evidence_section_code" value="' . \eel_accounts\Support\Utf8::html((string)($state['section_code'] ?? $section['section_code'] ?? '')) . '">'
        ...
        foreach ((array)($model['lifecycle'] ?? []) as $lifecycle) {
            $lifecycleRows .= '<tr><td>' . \eel_accounts\Support\Utf8::html((string)($lifecycle['created_at'] ?? '')) . '</td><td>'
                . \eel_accounts\Support\Utf8::html((string)($lifecycle['snapshot_hash'] ?? '')) . '</td></tr>';
        }
        return '<div class="helper"><span class="badge success">Frozen evidence</span> '
            . \eel_accounts\Support\Utf8::html((string)($section['section_code'] ?? '')) . ' · SHA-256 <code>' . \eel_accounts\Support\Utf8::html((string)($section['snapshot_hash'] ?? '')) . '</code>'
