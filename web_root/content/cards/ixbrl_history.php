        ...
        return '<div class="settings-stack">' . $table
            . ($developerOptions
                ? '<div class="form-row-actions"><form method="post" action="?page=disclosures" data-ajax="true">'
                    ...
                    . '<button class="button danger" type="submit" title="Developer only" data-chicken-check="true" data-chicken-title="Clean untransmitted iXBRL history" data-chicken-message="Remove local iXBRL history, filing approvals, submission drafts, unused evidence bundles, and obsolete Tax Audit snapshots that have never been transmitted. CT600 output metadata is cleared while its tax computation remains.<br><br>The newest approval and newest Tax Audit snapshot for each CT period are retained. Transmitted or in-flight filings remain protected. When eligible history exists, a full database backup is created immediately before and after cleanup. Generated files on disk are retained." data-chicken-confirm-text="Clean history" data-chicken-button-class="button danger">Clean Untransmitted History</button>'
                    . '</form></div>'
                : '')
            . '</div>';
    }
