...
final class _pl_expense_breakdownCard extends CardBaseFramework
{
    ...
    public function render(array $context): string
    {
        ...
        return '<div class="settings-stack">
            ' . $this->group('Cost of sales', (array)($breakdown['cost_of_sales'] ?? []), 'No cost of sales journals have been posted for this period.') . '
            ' . $this->group('Expenses', (array)($breakdown['expense'] ?? []), 'No expense journals have been posted for this period.') . '
        </div>';
    }

    private function group(string $title, array $rows, string $empty): string
    {
        if ($rows === []) {
