...
final class _tax_ct600_rim_mappingsCard extends CardBaseFramework
{
    ...
    private function renderProfiles(array $packages, array $profiles): string
    {
        ...
        foreach ($packages as $package) { $html .= '<option value="' . (int)$package['id'] . '">' . \eel_accounts\Support\Utf8::html((string)$package['form_version'] . ' / ' . (string)$package['artifact_version']) . '</option>'; }
    ...
    private function table(array $profiles): string
    {
        ...
        foreach ($profiles as $p) {
            ...
            $html .= '<tr><td>' . \eel_accounts\Support\Utf8::html((string)$p['profile_name'] . ' r' . (string)$p['revision_no']) . '</td><td>' . \eel_accounts\Support\Utf8::html((string)($p['rim_version'] ?? '') . ' / ' . (string)($p['rim_artifact_version'] ?? '')) . '</td><td>' . \eel_accounts\Support\Utf8::html((string)$p['status']) . '</td><td>' . \eel_accounts\Support\Utf8::html((string)$p['compatibility_status']) . '<br><span class="helper">' . count((array)($compatibility['unmapped_canonical_sources'] ?? [])) . ' canonical; ' . count((array)($compatibility['unmapped_required_targets'] ?? [])) . ' required target; ' . count((array)($compatibility['missing_targets'] ?? [])) . ' removed/changed</span></td><td>' . \eel_accounts\Support\Utf8::html(substr((string)$p['content_hash'], 0, 12)) . '</td><td>' . $this->actions($p) . '</td></tr>';
        }
