    ...
    public function render(array $context): string
    {
        ...
        foreach ((array)($workings['disposals_balancing'] ?? []) as $row) {
            $rows[] = [
                ...
                \eel_accounts\Ui\TaxCardRenderer::escape(HelperFramework::labelFromKey((string)($row['pool_type'] ?? ''), '_')),
                \eel_accounts\Ui\TaxCardRenderer::escape((string)($row['disposal_date'] ?? '')),
                \eel_accounts\Ui\TaxCardRenderer::escape(\eel_accounts\Ui\TaxCardRenderer::money($context, $row['disposal_value'] ?? ($row['disposal_proceeds'] ?? 0))),
                \eel_accounts\Ui\TaxCardRenderer::escape((string)($row['allowance_type'] ?? 'disposal')),
            ];
        }
        return \eel_accounts\Ui\TaxCardRenderer::header('capital_allowances')
            . \eel_accounts\Ui\TaxCardRenderer::table(['Asset', 'Pool', 'Disposal date', 'Disposal value', 'Tax treatment'], $rows, 'No disposal or balancing rows were found for this period.');
    }
}
