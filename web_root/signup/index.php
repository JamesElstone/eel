...
if ($request->isPost()) {
    if (!$sessionAuthenticationService->isValidCsrfToken((string)$request->post('csrf_token', ''))) {
        echo $renderer->errorPage($sessionAuthenticationService, [AccountCompletionService::PUBLIC_VERIFY_ERROR]);
        return;
    }
    ...
    if ($action === 'verify_identity') {
        if ($signupVerificationRateLimitService->isBlocked($request, $sessionAuthenticationService)) {
            ...
            echo $renderer->verificationPage($sessionAuthenticationService, [AccountCompletionService::PUBLIC_VERIFY_ERROR], $invite ?? []);
            return;
        }
        ...
        if (empty($result['success'])) {
            ...
            $invite = $completionService->verificationContext($completionSession);
            echo $renderer->verificationPage($sessionAuthenticationService, (array)($result['errors'] ?? []), $invite ?? []);
            return;
        }
...
if ($token !== '') {
    if ($signupTokenRateLimitService->isBlocked($request)) {
        echo $renderer->errorPage($sessionAuthenticationService, [AccountCompletionService::PUBLIC_TOKEN_ERROR]);
        return;
    }
    ...
    if (empty($result['success'])) {
        ...
        echo $renderer->errorPage($sessionAuthenticationService, (array)($result['errors'] ?? []));
        return;
    }

    echo $renderer->verificationPage($sessionAuthenticationService, [], (array)($result['invite'] ?? []));
    return;
}

$invite = $completionService->verificationContext($completionSession);
if ($invite !== null && $completionSession->isVerified()) {
    echo $renderer->completionPage($sessionAuthenticationService, $invite);
    return;
}

if ($completionSession->isValidated()) {
    $invite = $completionService->verificationContext($completionSession);
    echo $renderer->verificationPage($sessionAuthenticationService, [], $invite ?? []);
    return;
}
