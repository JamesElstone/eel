...
$harness->run(_source_accounts::class, static function (GeneratedServiceClassTestHarness $harness, _source_accounts $page): void {
    $harness->check(_source_accounts::class, 'maps field mapping account id from banking request context', static function () use ($harness, $page): void {
        $_GET = ['page' => 'source_accounts'];
        $_POST = [
        ...
        $reflection = new ReflectionMethod(_source_accounts::class, 'buildContext');
    ...
    $harness->check(_source_accounts::class, 'preserves selected mapping account after save requests', static function () use ($harness, $page): void {
        $_GET = ['page' => 'source_accounts'];
        $_POST = [
    ...
    $harness->check(_source_accounts::class, 'maps edit account id from banking edit request context', static function () use ($harness, $page): void {
        $_GET = ['page' => 'source_accounts'];
        $_POST = [
            'intent' => 'edit',
            'account_id' => '47',
    ...
    $harness->check(_source_accounts::class, 'maps edit account id from action query context', static function () use ($harness, $page): void {
        $_GET = ['page' => 'source_accounts'];
        $_POST = [];
        $_SERVER['REQUEST_METHOD'] = 'POST';
        $_SERVER['HTTP_X_REQUESTED_WITH'] = '';
