...
(new GeneratedServiceClassTestHarness())->run(
    ...
    static function (GeneratedServiceClassTestHarness $harness, \eel_accounts\Service\ProfitLossService $service): void {
        $harness->check(\eel_accounts\Service\ProfitLossService::class, 'reconciles asset disposal and unknown posted sources', static function () use ($harness, $service): void {
            ...
            try {
                ...
                $nominalId = (int)InterfaceDB::fetchColumn('SELECT id FROM nominal_accounts ORDER BY id LIMIT 1');

                profitLossSourceCoverageJournal($companyId, $periodId, $nominalId, 'asset_disposal', 'asset-disposal-' . $marker, 183.63);
                ...
                $harness->assertSame(1, (int)($coverage['asset_disposal']['journal_count'] ?? 0));
                ...
                $harness->assertSame(2, (int)($coverage['coverage_summary']['covered_journal_count'] ?? 0));
                $harness->assertSame(0, (int)($coverage['coverage_summary']['uncovered_journal_count'] ?? -1));
...
function profitLossSourceCoverageJournal(int $companyId, int $periodId, int $nominalId, string $sourceType, string $sourceRef, float $amount): void
{
    InterfaceDB::prepareExecute(
