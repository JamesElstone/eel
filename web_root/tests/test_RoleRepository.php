...
$harness->run(RoleRepository::class, static function (GeneratedServiceClassTestHarness $harness, RoleRepository $repository): void {
    ...
    $harness->check(RoleRepository::class, 'returns zero for an unknown user id', static function () use ($harness, $repository): void {
        $harness->assertSame(0, $repository->userRoleId(PHP_INT_MAX));
    });
});
