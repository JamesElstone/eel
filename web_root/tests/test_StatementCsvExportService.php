...
$harness->run(StatementCsvExportService::class, static function (GeneratedServiceClassTestHarness $harness, StatementCsvExportService $service): void {
    $harness->check(StatementCsvExportService::class, 'exports every mapping column with blanks for unmapped fields', static function () use ($harness, $service): void {
        ...
        $method = new ReflectionMethod(StatementCsvExportService::class, 'buildMappedCsv');
    ...
    $harness->check(StatementCsvExportService::class, 'committed transaction values override source CSV values', static function () use ($harness, $service): void {
        $directory = APP_ROOT . 'tests' . DIRECTORY_SEPARATOR . 'tmp';
        if (!is_dir($directory)) {
            mkdir($directory, 0777, true);
        }
    ...
    $harness->check(StatementCsvExportService::class, 'monthly export only includes rows for the selected transaction month', static function () use ($harness, $service): void {
        $directory = APP_ROOT . 'tests' . DIRECTORY_SEPARATOR . 'tmp';
        if (!is_dir($directory)) {
            mkdir($directory, 0777, true);
        }
    ...
    $harness->check(StatementCsvExportService::class, 'xlsx export includes an opaque row key and category dropdown sheet', static function () use ($harness, $service): void {
        $method = new ReflectionMethod(StatementCsvExportService::class, 'buildXlsx');
        $method->setAccessible(true);
    ...
    $harness->check(StatementCsvExportService::class, 'opaque row key is stable when editable fields change', static function () use ($harness): void {
        $first = StatementCsvExportService::exportRowKey(47, 216, 2, [
            'created' => '2026-02-01',
            'description' => 'Original description',
            'amount' => '12.34',
        ]);
        $second = StatementCsvExportService::exportRowKey(47, 216, 2, [
            'created' => '2026-02-01',
