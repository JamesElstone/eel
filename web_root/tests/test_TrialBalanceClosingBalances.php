        ...
        $harness->check(\eel_accounts\Service\TrialBalanceService::class, 'keeps period equality while reporting cumulative closing control balances', static function () use ($harness, $service): void {
            ...
            try {
                ...
                trialBalanceClosingJournal($companyId, $periodIds['prior'], '2024-12-31', 'tb-prior-' . $marker, [
                    [$nominals['1000'], 714.26, 0.00],
                    [$nominals['1200'], 253.00, 0.00],
                    [$nominals['2100'], 0.00, 1288.63],
                    [$nominals['3000'], 321.37, 0.00],
                ]);
                trialBalanceClosingJournal($companyId, $periodIds['current'], '2025-12-31', 'tb-current-' . $marker, [
                    [$nominals['1200'], 4620.83, 0.00],
                    [$nominals['1000'], 0.00, 590.12],
                    [$nominals['2100'], 0.00, 10873.46],
                    [$nominals['3000'], 6842.75, 0.00],
                ]);
                ...
                $harness->assertSame('124.14', number_format((float)($summary['bank_balance_total'] ?? 0), 2, '.', ''));
                $harness->assertSame('7288.26', number_format((float)($summary['director_loan_balance'] ?? 0), 2, '.', ''));
            } finally {
...
function trialBalanceClosingJournal(int $companyId, int $periodId, string $date, string $sourceRef, array $lines): void
{
    InterfaceDB::prepareExecute(
        ...
        ['company_id' => $companyId, 'period_id' => $periodId, 'source_type' => 'manual', 'source_ref' => $sourceRef, 'date' => $date, 'description' => 'Trial Balance closing fixture']
    );
    ...
    foreach ($lines as [$nominalId, $debit, $credit]) {
        InterfaceDB::prepareExecute(
            ...
            ['journal_id' => $journalId, 'nominal_id' => $nominalId, 'debit' => number_format($debit, 2, '.', ''), 'credit' => number_format($credit, 2, '.', ''), 'description' => 'Trial Balance closing fixture']
        );
    }
}
