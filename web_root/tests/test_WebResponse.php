...
$harness->check(WebResponse::class, 'builds HTML responses through the factory', function () use ($harness): void {
    $response = WebResponse::html('<p>Hello</p>', 201);
...
$harness->check(WebResponse::class, 'builds JSON responses through the factory', function () use ($harness): void {
    $response = WebResponse::json(['ok' => true], 202);
    $reflection = new ReflectionClass($response);
