    ...
    static function (GeneratedServiceClassTestHarness $harness, \eel_accounts\Service\YearEndTaxFreezeService $service): void {
        ...
        $harness->check(\eel_accounts\Service\YearEndTaxFreezeService::class, 'excludes filing-scope gates from the deterministic calculation hash', static function () use ($harness, $service): void {
            $scope = [
                'available' => true,
                'basis' => [
                    'scope_version' => 'ct-supplementary-scope-v2',
                    'answers' => [
                        'ct600a' => 'no',
                        'ct600l' => 'no',
                    ],
            ...
            $changedScope = $scope;
            $changedScope['basis']['answers']['ct600l'] = 'yes';

            $withoutScope = $service->build(49, 79, yearEndTaxFreezePeriods(), [], 2);
            $withScope = $service->build(49, 79, yearEndTaxFreezePeriods(), [], 2, $scope);
...
function yearEndTaxFreezePeriods(): array
{
    return [
        [
            ...
            'handles_ct600_turnover_rounding_residual' => false,
            'turnover_basis_version' => \eel_accounts\Service\CtPeriodTurnoverService::BASIS_VERSION,
            'accounting_period_turnover' => 10025.44,
            'accounting_period_box_145_turnover' => 10025,
            'turnover_reconciliation_difference' => 0,
